        ...
        <div className="space-y-2">
          {soldiers.map((s) => (
            <div key={s.id} className="flex items-center gap-2 rounded-xl border border-zinc-700 bg-zinc-800 px-3 py-2">
              ...
              <button
                type="button"
                onClick={() => onSetTrainee(s.id, !s.isTrainee)}
        ...
        <>
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-4 space-y-3">
            ...
            <div className="space-y-2 pt-2 border-t border-zinc-700">
              ...
              <select
                value={newUserRole}
                onChange={(e) => setNewUserRole(e.target.value as "admin" | "scheduler")}
                className="w-full rounded-xl border border-zinc-700 bg-zinc-800 px-3 py-2 text-sm text-white"
              >
          ...
          {/* Period settings */}
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-4 space-y-3">
            <h3 className="text-sm font-semibold text-zinc-200">תקופת שיבוץ</h3>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-xs text-zinc-400 block mb-1">התחלה</label>
                <input
                  type="date"
                  value={localSettings.periodStart}
                ...
                <div key={sd.id} className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    onClick={() => setLocalSettings((s) => ({
                      ...s,
                      specialDates: s.specialDates.filter((x) => x.id !== sd.id),
                    }))}
